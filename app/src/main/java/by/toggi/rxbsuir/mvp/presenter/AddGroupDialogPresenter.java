...
public class AddGroupDialogPresenter extends Presenter<AddGroupDialogView> {
  ...
  @Inject public AddGroupDialogPresenter(BsuirService service, StorIOSQLite storIOSQLite) {
    mService = service;
    mStorIOSQLite = storIOSQLite;
    mGroupListObservable = storIOSQLite.get()
        .listOfObjects(StudentGroup.class)
        .withQuery(Query.builder().table(GroupModel.TABLE_NAME).build())
        .prepare()
        .createObservable()
  ...
  private void getStudentGroupsFromNetwork() {
    mService.getStudentGroups()
        ...
        .subscribe(studentGroupPutResults -> Timber.d("Insert count: %d",
            studentGroupPutResults.numberOfInserts()), this::onError);
  }

  private Observable<PutResults<StudentGroup>> getStudentGroupPutObservable(
      List<StudentGroup> studentGroupList) {
    return mStorIOSQLite.put().objects(studentGroupList).prepare().createObservable();
  }
