...
public class AddEmployeeDialogPresenter extends Presenter<AddEmployeeDialogView> {
  ...
  @Inject public AddEmployeeDialogPresenter(BsuirService service, StorIOSQLite storIOSQLite) {
    mService = service;
    mStorIOSQLite = storIOSQLite;
    mEmployeeListObservable = mStorIOSQLite.get()
        .listOfObjects(Employee.class)
        .withQuery(Query.builder().table(EmployeeModel.TABLE_NAME).build())
        .prepare()
        .createObservable()
  ...
  private void getEmployeeListFromNetwork() {
    mService.getEmployees()
        ...
        .subscribe(employeePutResults -> Timber.d("Insert count: %d",
            employeePutResults.numberOfInserts()), this::onError);
  }

  private Observable<PutResults<Employee>> getEmployeePutObservable(List<Employee> employeeList) {
    return mStorIOSQLite.put().objects(employeeList).prepare().createObservable();
  }
