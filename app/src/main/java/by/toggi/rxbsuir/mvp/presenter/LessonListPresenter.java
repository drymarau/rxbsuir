...
public class LessonListPresenter extends Presenter<LessonListView> {
  ...
  @Override public void onCreate() {
    ...
    mSubscription = getLessonListObservable(getLessonQuery()).subscribe(lessons -> {
      if (lessons.size() > 0) {
  ...
  private LessonEntry.Query getLessonQuery() {
    ...
    switch (mType) {
      case TODAY:
        ...
        return builder.weekNumber(Utils.getWeekNumber(today)).weekDay(today.getDayOfWeek()).build();
      case TOMORROW:
  ...
  private Observable<List<Lesson>> getLessonListObservable(LessonEntry.Query query) {
    return mSyncId == null ? Observable.empty() : mStorIOSQLite.get()
        ...
        .withQuery(Query.builder().table(LessonModel.TABLE_NAME).where(query.toString()).build())
        .prepare()
        .createObservable()
