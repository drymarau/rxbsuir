...
public class LessonDetailPresenter extends Presenter<LessonDetailView> {
  ...
  @Inject public LessonDetailPresenter(StorIOSQLite storIOSQLite) {
  ...
  public void setLesson(Lesson lesson) {
    ...
    mSubscription = mStorIOSQLite.get()
        ...
        .withQuery(Query.builder()
            ...
            .where(LessonModel._ID + "= '" + mLesson.getId() + "'")
            .build())
        .prepare()
        .createObservable()
        .observeOn(AndroidSchedulers.mainThread())
  ...
  private List<DetailItem> getDetailItemList(Lesson lesson) {
    ...
    detailItemList.add(DetailItem.newInstance(DetailItem.Type.TIME,
        lesson.getPrettyLessonTimeStart() + '-' + lesson.getPrettyLessonTimeEnd(), true));
    detailItemList.add(DetailItem.newInstance(DetailItem.Type.WEEKDAY, lesson.getPrettyWeekday(),
        lesson.getPrettyWeekNumberList(), true));
    if (lesson.getAuditoryList() != null && !lesson.getAuditoryList().isEmpty()) {
      detailItemList.add(
          DetailItem.newInstance(DetailItem.Type.AUDITORY, lesson.getPrettyAuditoryList(), true));
    }
    if (lesson.isGroupSchedule()) {
      for (int i = 0, size = lesson.getEmployeeList().size(); i < size; i++) {
        ...
        detailItemList.add(
            DetailItem.newInstance(DetailItem.Type.EMPLOYEE, employee.toString(), i == 0));
      }
      if (lesson.getStudentGroupList().size() > 1) {
        for (int i = 0, size = lesson.getStudentGroupList().size(); i < size; i++) {
          ...
          detailItemList.add(DetailItem.newInstance(DetailItem.Type.GROUP, group, i == 0));
        }
      }
    } else {
      for (int i = 0, size = lesson.getStudentGroupList().size(); i < size; i++) {
    ...
    if (lesson.getNote() != null && !lesson.getNote().isEmpty()) {
      detailItemList.add(DetailItem.newInstance(DetailItem.Type.NOTE, lesson.getNote(), true));
    }
