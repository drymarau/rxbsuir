...
public class SchedulePresenter extends Presenter<ScheduleView> {
      ...
      Observable.concat(
          ...
          mStorIOSQLite.get()
              ...
              .withQuery(Query.builder()
                  ...
                  .where(LessonEntry.Query.builder(syncId, isGroupSchedule).build().toString())
                  .build())
              .prepare()
              .createObservable()
  ...
  public void remove(String syncId, Boolean isGroupSchedule) {
    ...
    Observable.concat(getDeleteSyncIdObservable(syncId, isGroupSchedule),
        isGroupSchedule ? getCacheGroupObservable(false) : getCacheEmployeeObservable(false))
        .doOnError(this::onError)
        .subscribe();
  }

  private Observable<DeleteResult> getDeleteSyncIdObservable(String syncId,
  ...
  private void onNetworkSuccess(List<Lesson> lessonList, boolean isGroupSchedule) {
    Observable.concat(getDeleteSyncIdObservable(mSyncId, isGroupSchedule),
        mStorIOSQLite.put().objects(lessonList).prepare().createObservable(),
  ...
  private void onError(Throwable throwable) {
    ...
    if (isViewAttached()) {
      if (throwable.getMessage() != null && throwable.getMessage()
          .contains("org.simpleframework.xml.core.ValueRequiredException")) {
  ...
  private List<Lesson> transformScheduleToLesson(ScheduleModel model, boolean isGroupSchedule) {
    ...
    for (Schedule schedule : model.schedule) {
      lessonList.add(new Lesson(mSyncId, schedule, Utils.convertWeekdayToDayOfWeek(model.weekDay),
          isGroupSchedule));
    }
    return lessonList;
  }

  private Observable<List<Lesson>> getLessonListFromNetworkObservable(@NonNull String syncId,
      boolean isGroupSchedule) {
    Observable<ScheduleJsonModels> scheduleXmlModelsObservable =
        isGroupSchedule ? mService.getGroupSchedule(syncId.replace("М", "M"))
            : mService.getEmployeeSchedule(syncId);
    return scheduleXmlModelsObservable.observeOn(AndroidSchedulers.mainThread())
        .subscribeOn(Schedulers.io())
        .flatMap(scheduleJsonModels -> Observable.from(scheduleJsonModels.schedules))
        .flatMap(scheduleModel -> Observable.from(
            transformScheduleToLesson(scheduleModel, isGroupSchedule)))
        .toList()
