...
public class Utils {
  ...
  public static void setNotificationAlarm(Context context, LocalTime localTime) {
    ...
    getAlarmManager(context).setInexactRepeating(AlarmManager.RTC_WAKEUP,
        convertLocalDateTimeToMillis(dateTime), TimeUnit.MILLISECONDS.convert(1, TimeUnit.DAYS),
        pendingIntent);
  ...
  public static void setWidgetUpdateAlarm(Context context) {
    if (getAppWidgetCount(context) > 0) {
      ...
      manager.setInexactRepeating(AlarmManager.RTC_WAKEUP, convertLocalDateTimeToMillis(dateTime),
