...
@PaperParcel public class Lesson implements Parcelable {

  public static final Creator<Lesson> CREATOR = PaperParcelLesson.CREATOR;
  ...
  public Lesson(@Nullable Long _id, @NonNull String syncId, List<String> auditoryList,
      List<Employee> employeeList, LocalTime lessonTimeStart, LocalTime lessonTimeEnd,
      String lessonType, String note, int numSubgroup, List<String> studentGroupList,
      String subject, List<Integer> weekNumberList, DayOfWeek weekday, boolean isGroupSchedule) {
    this._id = _id;
    this.syncId = syncId;
  ...
  public Lesson(@NonNull String syncId, Schedule schedule, DayOfWeek weekday,
      boolean isGroupSchedule) {
    this._id = null;
    this.syncId = syncId;
  ...
  public String getPrettyLesson() {
    if (getPrettyAuditoryList().isEmpty()) {
      return String.format("%s-%s %s (%s)", getPrettyLessonTimeStart(), getPrettyLessonTimeEnd(),
          getSubjectWithSubgroup(), getLessonType());
    } else {
      return String.format("%s-%s %s (%s) @ %s", getPrettyLessonTimeStart(),
          getPrettyLessonTimeEnd(), getSubjectWithSubgroup(), getLessonType(),
  ...
  @Override public void writeToParcel(Parcel dest, int flags) {
    PaperParcelLesson.writeToParcel(this, dest, flags);
  }
}
