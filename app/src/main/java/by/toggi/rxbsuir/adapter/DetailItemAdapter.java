...
public class DetailItemAdapter extends RecyclerView.Adapter<DetailItemAdapter.ViewHolder>
    implements DividerItemDecoration.Provider {
  ...
  @Override public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    switch (viewType) {
      ...
      case VIEW_TYPE_DETAIL_SUMMARY:
        view = LayoutInflater.from(parent.getContext())
            .inflate(R.layout.list_item_detail_summary, parent, false);
        break;
      default:
  ...
  @Override public void onBindViewHolder(ViewHolder holder, int position) {
    ...
    if (holder.mSummary != null) {
      holder.mSummary.setText(detailItem.getSummary() == null ? mContext.getString(R.string.weekly)
          : mContext.getResources()
              .getQuantityString(R.plurals.week, detailItem.getSummary().length(),
  ...
  @Override public boolean hasDivider(@NotNull Resources resources, int position) {
    if (position == getItemCount() - 1) {
      return false;
    }
    DetailItem.Type currentType = mDetailItemList.get(position).getType();
    DetailItem.Type nextType = mDetailItemList.get(position + 1).getType();
